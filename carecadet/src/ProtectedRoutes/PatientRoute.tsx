...
const PatientRoute = ({ children, getData }: Props) => {
  const navigate = useNavigate();
  const location=useLocation().pathname.split("/")[1]

  const dispatch = useAppDispatch();
  const userID = useAppSelector((state) => state.providerAuth.login.userID);
  const authUser = useAppSelector((state) => state.patientAuth);
...
//   };
  ...
  let isAuth =
    path==="patient" &&
    authUser.patientLogin.userType==="PATIENT"&&
    (authUser.patientLogin !== null || undefined || "") &&
    authUser.patientLogoutButton;
